...
def joint_angle_decoder(joint_angles, differential_ik, robot):
    joint_descriptor = []
    for i in range(7):
        link_name = "iiwa_link_{}".format(i+1)
        X_joint = differential_ik.ForwardKinematics(joint_angles, robot.GetFrameByName(link_name)).matrix()
        rot = X_joint[0:3, 0:3]
        trans = X_joint[0:3, 3]
        joint_descriptor.extend(np.append(rot.flatten(), trans))
    assert(len(joint_descriptor) == 7*12)
    return joint_descriptor

def predict_clf_and_partials(model, image, joint_angles, differential_ik, robot):
    #image = Image.fromarray(np.uint8(cm.gist_earth(image*255)))
    #image = plt.imshow(image.data)._rgba_cache
    image = Image.fromarray(image.data, 'RGB')
    transform = transforms.Compose([
        transforms.Resize(224),
        transforms.ToTensor(),
        transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])

    image_tensor = transform(image).type(torch.cuda.FloatTensor)
    image_tensor = image_tensor.unsqueeze_(0)
    input_image = Variable(image_tensor)

    input_joint_descriptor = torch.from_numpy(np.array(joint_angle_decoder(joint_angles, differential_ik, robot)))
    input_joint_descriptor = input_joint_descriptor.type(torch.cuda.FloatTensor)
    input_joint_descriptor = input_joint_descriptor.unsqueeze_(0)
    output = model(input_image, input_joint_descriptor)
    return output.cpu().detach().numpy()[0]

def run_trial(image_num, model):
    ...
    while t - start_t < 40:
        ...
        print('jacobian: \n{}'.format(jacobian))
        real_pdV = np.zeros(num_joints)
        ang_jacobian = jacobian[0:3, 0:7]
        trans_jacobian = jacobian[3:6, 0:7]

        for i in range(7):
            rot_jacobian = skew(ang_jacobian[0:3, i]) @ R_H
            real_pdV[i] = -2 * np.trace(R_G @ rot_jacobian) + 2 * (p_H - p_G) @ trans_jacobian[0:3, i]

        print('real_pdV', real_pdV)

        q_prev = station.GetIiwaPosition(station_context)
        img = rgb_image_visualizer.get_color_image()
        clf_and_partials = predict_clf_and_partials(model, img, q_prev, differential_ik, robot)
        pdV = clf_and_partials[1:]
        print(clf_and_partials)
...
def main():
    ...
    model_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'model/models/0010.pth.tar')
    ...
    while image_num < 100000:
        image_num = run_trial(image_num, model)
        gc.collect()
